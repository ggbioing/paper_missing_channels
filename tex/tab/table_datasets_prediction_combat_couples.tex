...
\caption{
% Age MMSE
Mean Squared Error (MSE, the lower the better) measured on test dataset views (clinical scores and imaging derived phenotypes) predicted with the Multi-Channel VAE (mcvae) and our variant (ours).
$5$-folds cross-validation results shown as as average (standard deviation).
Models were trained on all the available views in the training dataset, independently of their presence in the testing dataset.
...
There are $12$ cases in the latter condition that could be fitted with our model.
In $7$ of them we measure a better performance with respect to the first condition (\S), and with respect to the second condition (\dag).
In all the cases our model performs either similarly or statistically better than the mcvae, with alpha levels at $0.05$ (*), $0.01$ (**), and $0.001$ (***).
}
\label{tab:features}
\resizebox{\textwidth}{!}{
