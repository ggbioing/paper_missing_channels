...
\caption{
% Age MMSE
Mean Squared Reconstruction Error (the lower the better) measured on test dataset views (clinical scores and imaging derived phenotypes) predicted with the Multi-Channel VAE (MCVAE) and the Multi Task MCVAE (MT-MCVAE).
$5$-folds cross-validation results shown as average (standard deviation).
Models were trained on all the available views in the training dataset, independently of their presence in the testing dataset.
...
In all cases the MT-MCVAE performs either similarly or statistically better than the MCVAE, with alpha levels at $0.05$ (*), $0.01$ (**), and $0.001$ (***).
}
\label{tab:features}
\resizebox{\textwidth}{!}{
\begin{tabular}{lllllllllllll}
\toprule
                 &       & view            & \multicolumn{2}{c}{clin}              & \multicolumn{2}{c}{MRI}             & \multicolumn{2}{c}{FDG}             & \multicolumn{2}{c}{AV45}            & \multicolumn{2}{c}{AV1451}      \\
...
\caption{
% Age MMSE
Mean Squared Reconstruction Error (the lower the better) measured on test dataset views (clinical scores and imaging derived phenotypes) predicted with our model.
$5$-folds cross-validation results shown as as average (standard deviation).
Models were trained on all the available views in the training dataset, independently of their presence in the testing dataset.
...
2) when models trained on one dataset are tested on another dataset, or Single Task with External Benchmark (STEB) case;
3) when models are trained on all the available datasets except the testing one, or Multi Task Learning (MTL).
We measure a better performance in the MTL condition with respect to the STIB (\S) in $7/12$ of cases,
and in $10/12$ of cases with respect to the average STEB (\dag) experiments.
}
\label{tab:features_mtl}
