...
\end{aligned}
\end{equation}
We define the encoder $\qz = \q{\z|\x_{d,n,w}, \phib_w} = q_{d,n,w}(\z)$ and its variational parameter $\phib_w$ to depend on a specific view $w$, and, to force a link among views, we impose the inequality \eqnref{eq:LB} to hold for any encoding $q_{\cdot,\cdot,w}(\z)$.
We can hence sum the right and left hand sides of \eqnref{eq:LB} across the $V_{d,n}$ views to give the lower bound:
