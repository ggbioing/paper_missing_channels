...
\subsubsection{Non linear parameterization}
...
Typically it is done by stacking many linear or convolution layers, interleaved with non-linear activation functions such as the sigmoid and the hyperbolic tangent.

Potentially, the use of non linear networks can provide better results.
Unfortunately the design choices that the model user must make
(for example the number of layers, the sizes of hidden dimensions, the activation functions, \etc),
can be highly dependent on her/his field of work and expertise.
%
To avoid this bias, in this work the bulk of our experiments is based on linear parametric mappings from the observation space towards the latent space, and \textit{vice versa}.
Nevertheless, for the sake of completeness, we will also provide some results based on non linear modeling experiments, starting from the following conceptual case.
